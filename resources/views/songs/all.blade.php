...
?>
<section class='py-5'>
    <div class='container'>@if(session('success1') OR session('success2'))
        ...
        <div class="card border-dark mb-3">
            ...
            <div class="card-body text-dark">
                ...
                <p class="card-text">Artist:
                    @if(empty($latestsong->artistid)){{$latestsong->artist}} @else <a target="_blank" href='https://anilist.co/staff/{{$latestsong->artistid}}'>{{$latestsong->artist}}</a> @endif
                    <br>Anime:
                    <a href="https://anilist.co/anime/{{ $latestsong->animeid }}">{{ $latestsong->anime }}</a>
                </p>
            </div>
        ...
        <br>
        <form>
            <input class="round" id='myInput1' onkeyup='myFunction1()' placeholder='Search for anime..' type='text'>
        </form>
        <form>
            <input class="round" id='myInput2' onkeyup='myFunction2()' placeholder='Search for songs..' type='text'>
        </form>
        <form>
            <input class="round" id='myInput3' onkeyup='myFunction3()' placeholder='Search for artists..' type='text'>
        </form> 
        <div class="progress">
        ...
        <table id='myTable'>
            ...
            <tr>
                ...
                @notmobile<td>@if(empty($song->artistid)){{$song->artist}} @else
                    <a target="_blank" href='https://anilist.co/staff/{{$song->artistid}}'>{{$song->artist}}</a> @endif</td>@endnotmobile
